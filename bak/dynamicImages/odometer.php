...
if(!isset($_GET['draw'])){
  ...
  $m=array('l'=>45,'r'=>15,'t'=>15,'b'=>30);//margins
  ...
  imagestring($img,2,5,5
       ,substr((string)intval($g_bounds['max_y']),0,2)
       ,$black);
  for($i=0;$i<=$div['v'];$i++){
    imageline ($img, $or['x'],$tr['y']+$i*$space, $or['x']+3,$tr['y']+$i*$space,$black);//horizontal
    imagestring($img,2,$or['x']-25,$tr['y']+$i*$space-8
       ,substr((string)intval($g_bounds['max_y']-($i)*$yg_range/$div['v']),2)
       ,$black);
  }
